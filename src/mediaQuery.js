...
class MediaQuery {
  ...
  rebuild() {
    this.selectors = {
      ...
      smOnly: `@media (min-width: ${
        this.breakpoints.sm
      }px) and (max-width: ${this.breakpoints.md - 1}px)`,
      mdOnly: `@media (min-width: ${
        this.breakpoints.md
      }px) and (max-width: ${this.breakpoints.lg - 1}px)`,
      lgOnly: `@media (min-width: ${
        this.breakpoints.lg
      }px) and (max-width: ${this.breakpoints.xl - 1}px)`,
      xlOnly: `@media (min-width: ${
        this.breakpoints.xl
      }px) and (max-width: ${this.breakpoints.xxl - 1}px)`,
      xxlOnly: `@media (min-width: ${this.breakpoints.xxl}px)`
    };

    Object.keys(this.selectors).forEach((screenName) => {
      this[screenName] = (...args) => (screenName === 'xxs'
        ? css(...args)
        : css`
